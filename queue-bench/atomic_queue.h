...
template <bool minimize_contention, unsigned array_size,
          size_t elements_per_cache_line>
struct GetIndexShuffleBits {
  static int constexpr bits =
      GetCacheLineIndexBits<elements_per_cache_line>::value;
...
template <int BITS> constexpr unsigned remap_index(unsigned index) noexcept {
  return remap_index_with_mix<BITS>(index, (index ^ (index >> BITS)) &
                                               ((1u << BITS) - 1));
}
...
template <class T> constexpr T decrement(T x) noexcept { return x - 1; }

template <class T> constexpr T increment(T x) noexcept { return x + 1; }
...
protected:
  ...
  static void do_push_atomic(T element, std::atomic<T> &q_element) noexcept {
    assert(element != NIL);
    if (Derived::spsc_) {
      while (__glibc_unlikely(q_element.load(X) != NIL))
        if (Derived::maximize_throughput_)
          spin_loop_pause();
      q_element.store(element, R);
    } else {
      for (T expected = NIL; __glibc_unlikely(
               !q_element.compare_exchange_strong(expected, element, R, X));
           expected = NIL) {
        do
  ...
  template <class T>
  static T do_pop_any(std::atomic<unsigned char> &state,
                      T &q_element) noexcept {
    if (Derived::spsc_) {
      while (__glibc_unlikely(state.load(A) != STORED))
        if (Derived::maximize_throughput_)
          spin_loop_pause();
      ...
      for (;;) {
        ...
        if (__glibc_likely(
                state.compare_exchange_strong(expected, LOADING, A, X))) {
          T element{std::move(q_element)};
          state.store(EMPTY, R);
          return element;
        }
  ...
  static void do_push_any(U &&element, std::atomic<unsigned char> &state,
                          T &q_element) noexcept {
    if (Derived::spsc_) {
      while (__glibc_unlikely(state.load(A) != EMPTY))
        if (Derived::maximize_throughput_)
          spin_loop_pause();
      ...
      for (;;) {
        ...
        if (__glibc_likely(
                state.compare_exchange_strong(expected, STORING, A, X))) {
          q_element = std::forward<U>(element);
          state.store(STORED, R);
          return;
        }
...
public:
    ...
    if (Derived::spsc_) {
      ...
      do {
        if (static_cast<int>(head - tail_.load(X)) >=
            static_cast<int>(static_cast<Derived &>(*this).size_))
          return false;
      } while (__glibc_unlikely(!head_.compare_exchange_strong(
          head, head + 1, A, X))); // This loop is not FIFO.
    }
  ...
  template <class T> bool try_pop(T &element) noexcept {
    ...
    if (Derived::spsc_) {
      ...
      do {
        if (static_cast<int>(head_.load(X) - tail) <= 0)
          return false;
      } while (__glibc_unlikely(!tail_.compare_exchange_strong(
          tail, tail + 1, A, X))); // This loop is not FIFO.
    }
  ...
  bool was_full() const noexcept {
    return was_size() >=
           static_cast<int>(static_cast<Derived const &>(*this).size_);
  }
...
template <class T, unsigned SIZE, T NIL = T{}, bool MINIMIZE_CONTENTION = true,
...
class AtomicQueue : public AtomicQueueCommon<
  ...
  alignas(CACHE_LINE_SIZE)
      std::atomic<T> elements_[size_] = {}; // Empty elements are NIL.

  T do_pop(unsigned tail) noexcept {
    std::atomic<T> &q_element =
        details::map<SHUFFLE_BITS>(elements_, tail % size_);
    return Base::template do_pop_atomic<T, NIL>(q_element);
  }

  void do_push(T element, unsigned head) noexcept {
    std::atomic<T> &q_element =
        details::map<SHUFFLE_BITS>(elements_, head % size_);
...
public:
  ...
  AtomicQueue() noexcept {
    assert(std::atomic<T>{NIL}
               .is_lock_free()); // This queue is for atomic elements only.
                                 // AtomicQueue2 is for non-atomic ones.
    if (T{} != NIL)
...
class AtomicQueue2 : public AtomicQueueCommon<
  ...
  static constexpr int SHUFFLE_BITS =
      details::GetIndexShuffleBits<MINIMIZE_CONTENTION, size_,
                                   CACHE_LINE_SIZE / sizeof(State)>::value;
  static constexpr bool total_order_ = TOTAL_ORDER;
  static constexpr bool spsc_ = SPSC;
  static constexpr bool maximize_throughput_ = MAXIMIZE_THROUGHPUT;
  ...
  template <class U> void do_push(U &&element, unsigned head) noexcept {
    ...
    Base::template do_push_any(std::forward<U>(element), states_[index],
                               elements_[index]);
  }
...
class AtomicQueueB
  ...
  T do_pop(unsigned tail) noexcept {
    std::atomic<T> &q_element =
        details::map<SHUFFLE_BITS>(elements_, tail & (size_ - 1));
    return Base::template do_pop_atomic<T, NIL>(q_element);
  }

  void do_push(T element, unsigned head) noexcept {
    std::atomic<T> &q_element =
        details::map<SHUFFLE_BITS>(elements_, head & (size_ - 1));
    Base::template do_push_atomic<T, NIL>(element, q_element);
  }
...
public:
  ...
  AtomicQueueB(unsigned size)
      : size_(std::max(details::round_up_to_power_of_2(size),
                       1u << (SHUFFLE_BITS * 2))),
        elements_(AllocatorElements::allocate(size_)) {
    assert(std::atomic<T>{NIL}
               .is_lock_free()); // This queue is for atomic elements only.
                                 // AtomicQueueB2 is for non-atomic ones.
  ...
  AtomicQueueB(AtomicQueueB &&b) noexcept
      : Base(static_cast<Base &&>(b)),
        ...
        ,
        size_(b.size_), elements_(b.elements_) {
    b.size_ = 0;
    b.elements_ = 0;
  }
  ...
  ~AtomicQueueB() noexcept {
    if (elements_)
      AllocatorElements::deallocate(
          elements_, size_); // TODO: This must be noexcept, static_assert that.
  }
  ...
  friend void swap(AtomicQueueB &a, AtomicQueueB &b) { a.swap(b); }
};
...
class AtomicQueueB2
    : public AtomicQueueCommon<
      ...
      private A,
      private std::allocator_traits<A>::template rebind_alloc<
          std::atomic<uint8_t>> {
  using Base = AtomicQueueCommon<
      AtomicQueueB2<T, A, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>>;
  using State = typename Base::State;
  ...
  using AllocatorStates = typename std::allocator_traits<
      A>::template rebind_alloc<std::atomic<uint8_t>>;
...
public:
  ...
  AtomicQueueB2(AtomicQueueB2 &&b) noexcept
      : Base(static_cast<Base &&>(b)),
        AllocatorElements(static_cast<AllocatorElements &&>(
            b)) // TODO: This must be noexcept, static_assert that.
        ,
        AllocatorStates(static_cast<AllocatorStates &&>(
            b)) // TODO: This must be noexcept, static_assert that.
        ,
        size_(b.size_), states_(b.states_), elements_(b.elements_) {
    b.size_ = 0;
    b.states_ = 0;
    b.elements_ = 0;
  }
  ...
  ~AtomicQueueB2() noexcept {
    if (elements_) {
      ...
      AllocatorElements::deallocate(
          elements_, size_); // TODO: This must be noexcept, static_assert that.
      AllocatorStates::deallocate(
          states_, size_); // TODO: This must be noexcept, static_assert that.
    }
  }
  ...
  void swap(AtomicQueueB2 &b) noexcept {
    ...
    swap(static_cast<AllocatorElements &>(*this),
         static_cast<AllocatorElements &>(b));
    swap(static_cast<AllocatorStates &>(*this),
         static_cast<AllocatorStates &>(b));
    swap(size_, b.size_);
    swap(states_, b.states_);
    swap(elements_, b.elements_);
  }

  friend void swap(AtomicQueueB2 &a, AtomicQueueB2 &b) noexcept { a.swap(b); }
};
